...
The Student Retention Add-in communicates with the Chrome Extension using the `window.postMessage()` API. The background script (`commands/background-service.js`) automatically syncs the Master List data when the extension is detected.
