...
function Comment({ entry, searchTerm, index, onContextMenu, delete: deleteFunction, save: saveFunction }) {
  ...
  // bold search term in comment (for non-quote or fallback)
  ...
  if (!hasQuoteTag || !quoteText) {
    if (isLdaWithDate && entry.comment && ldaDateRegex) {
      ...
      // bold "Tomorrow", "next week", weekdays, and weekends in the resulting parts
      commentContent = parts.flatMap(part => boldLdaKeywords(part));
               ...
               return (
                 <span
                   ...
                   style={{
                     ...
                     whiteSpace: 'nowrap',
                     cursor: 'pointer',
                     userSelect: 'none' // prevent bolding
                   }}
                   onClick={e => {
...
// Helper to bold LDA keywords (exported for reuse)
export function boldLdaKeywords(text) {
  if (typeof text !== "string") return text;

  // Use ldaKeywords to determine tokens that should be bolded.
  // ldaKeywords returns { dates: [], keywords: [] } where `keywords` are the raw matched tokens.
  const { keywords } = ldaKeywords(text || '');
  if (!keywords || keywords.length === 0) return text;

  // Sort tokens by length descending to avoid partial matches (e.g., 'day' inside 'Monday').
  const tokens = Array.from(new Set(keywords)).sort((a, b) => b.length - a.length);
